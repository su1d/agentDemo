  ...
  <div ref="container" class="iso-office-container">
    ...
    <div class="office-toolbar">
      ...
      <div class="toolbar-left">
        ...
        <span class="toolbar-title">Agent 动物办公室</span>
...
// ---- Stylized 3D animal builders (game-ready aesthetic) ----
...
function createStylizedOwl(color) {
  ...
  const bodyMat = new THREE.MeshStandardMaterial({
    ...
    color: c, roughness: 0.3, metalness: 0.3,

    emissive: c, emissiveIntensity: 0.04,

  })

  const darkMat = new THREE.MeshStandardMaterial({ color: 0x1a1a2e, roughness: 0.5 })

  const beakMat = new THREE.MeshStandardMaterial({ color: 0xff8c42, roughness: 0.4, metalness: 0.1 })

  const eyeWhite = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xffffff, emissiveIntensity: 0.15 })



  // Body - rounded capsule

  const body = new THREE.Mesh(new THREE.CapsuleGeometry(0.09, 0.06, 8, 10), bodyMat)

  body.position.y = 0.06; body.scale.set(1, 1.1, 0.85); g.add(body)



  // Head - slightly flattened sphere

  const head = new THREE.Mesh(new THREE.SphereGeometry(0.08, 10, 10), bodyMat)

  head.scale.set(1, 0.9, 0.85); head.position.set(0, 0.16, -0.01); g.add(head)



  // Eyes - large cute circles with pupil

  for (let s = -1; s <= 1; s += 2) {

    const eye = new THREE.Mesh(new THREE.SphereGeometry(0.035, 10, 10), eyeWhite)

    eye.scale.set(1, 0.9, 0.5); eye.position.set(s * 0.045, 0.18, -0.07); g.add(eye)

    const pupil = new THREE.Mesh(new THREE.SphereGeometry(0.02, 8, 8), new THREE.MeshStandardMaterial({ color: 0x111111 }))

    pupil.position.set(s * 0.045, 0.18, -0.055); g.add(pupil)

    // Eye highlight

    const hl = new THREE.Mesh(new THREE.SphereGeometry(0.008, 6, 6), new THREE.MeshStandardMaterial({ color: 0xffffff }))

    hl.position.set(s * 0.05, 0.19, -0.06); g.add(hl)
  ...
  const beak = new THREE.Mesh(new THREE.ConeGeometry(0.025, 0.04, 4), beakMat)

  beak.rotation.x = 0.4; beak.position.set(0, 0.14, -0.11); g.add(beak)



  // Wings - teardrop/leaf shapes (slightly flattened spheres)

  for (let s = -1; s <= 1; s += 2) {

    const wing = new THREE.Mesh(new THREE.SphereGeometry(0.045, 8, 8), bodyMat)

    wing.scale.set(0.35, 0.75, 0.5); wing.position.set(s * 0.12, 0.06, 0); g.add(wing)

  }



  // Ear tufts - tiny cones

  for (let s = -1; s <= 1; s += 2) {

    const t = new THREE.Mesh(new THREE.ConeGeometry(0.015, 0.05, 4), bodyMat)

    t.position.set(s * 0.03, 0.22, -0.02); g.add(t)
...
function createStylizedFox(color) {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: c, roughness: 0.3, metalness: 0.25, emissive: c, emissiveIntensity: 0.04 })
  ...
  const body = new THREE.Mesh(new THREE.CapsuleGeometry(0.08, 0.06, 8, 10), mat)

  body.position.y = 0.06; body.scale.set(1.05, 1, 0.75); g.add(body)



  // Head

  const head = new THREE.Mesh(new THREE.SphereGeometry(0.065, 10, 10), mat)

  head.scale.set(1, 0.85, 0.85); head.position.set(0, 0.13, -0.09); g.add(head)



  // Snout (elongated cone)

  const snout = new THREE.Mesh(new THREE.ConeGeometry(0.025, 0.08, 6), mat)

  snout.rotation.x = -0.15; snout.position.set(0, 0.12, -0.16); g.add(snout)
  ...
  nose.position.set(0, 0.12, -0.2); g.add(nose)



  // Ears - big pointy triangles

  for (let s = -1; s <= 1; s += 2) {

    const ear = new THREE.Mesh(new THREE.ConeGeometry(0.03, 0.07, 4), mat)

    ear.position.set(s * 0.04, 0.185, -0.06); ear.scale.set(1, 1, 0.7); g.add(ear)

    const ei = new THREE.Mesh(new THREE.ConeGeometry(0.015, 0.04, 4), innerEarMat)

    ei.position.set(s * 0.04, 0.195, -0.06); g.add(ei)
  ...
  for (let s = -1; s <= 1; s += 2) {
    ...
    const e = new THREE.Mesh(new THREE.SphereGeometry(0.016, 8, 8), eyeMat)

    e.position.set(s * 0.025, 0.145, -0.13); g.add(e)

    const hl = new THREE.Mesh(new THREE.SphereGeometry(0.006, 6, 6), new THREE.MeshStandardMaterial({ color: 0xffffff }))

    hl.position.set(s * 0.028, 0.15, -0.135); g.add(hl)
  ...
  const tail = new THREE.Mesh(new THREE.CapsuleGeometry(0.025, 0.08, 6, 8), mat)

  tail.rotation.z = 0.5; tail.rotation.x = 0.4; tail.position.set(0, 0.06, 0.14); g.add(tail)

  const tip = new THREE.Mesh(new THREE.SphereGeometry(0.018, 8, 8), new THREE.MeshStandardMaterial({ color: 0xffffff }))

  tip.position.set(0.02, 0.08, 0.2); g.add(tip)
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    for (let front = -1; front <= 1; front += 2) {
      ...
      const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.018, 0.035, 6), mat)

      leg.position.set(side * 0.04, 0.018, front * 0.06); g.add(leg)
...
function createStylizedDolphin(color) {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: c, roughness: 0.15, metalness: 0.45, emissive: c, emissiveIntensity: 0.03 })



  // Body - sleek capsule

  const body = new THREE.Mesh(new THREE.CapsuleGeometry(0.06, 0.16, 8, 10), mat)

  body.scale.set(1, 0.8, 0.7); body.rotation.x = Math.PI * 0.08; body.position.y = 0.04; g.add(body)



  // Beak

  const beak = new THREE.Mesh(new THREE.ConeGeometry(0.018, 0.08, 6), mat)

  beak.rotation.x = -0.2; beak.position.set(0, 0.05, -0.18); g.add(beak)



  // Dorsal fin

  const df = new THREE.Mesh(new THREE.ConeGeometry(0.02, 0.06, 4), mat)

  df.position.set(0, 0.1, -0.01); df.scale.set(0.6, 1, 0.3); g.add(df)



  // Tail flukes (two cones)

  for (let s = -1; s <= 1; s += 2) {

    const fluke = new THREE.Mesh(new THREE.ConeGeometry(0.018, 0.05, 4), mat)

    fluke.rotation.z = s * 0.4; fluke.position.set(s * 0.04, 0.01, 0.17); g.add(fluke)
  ...
  const eye = new THREE.Mesh(new THREE.SphereGeometry(0.012, 8, 8), new THREE.MeshStandardMaterial({ color: 0x111111 }))

  eye.position.set(0.025, 0.065, -0.13); g.add(eye)
  ...
  for (let s = -1; s <= 1; s += 2) {
    ...
    const pf = new THREE.Mesh(new THREE.ConeGeometry(0.012, 0.04, 4), mat)

    pf.rotation.z = s * 0.4; pf.rotation.x = -0.3

    pf.position.set(s * 0.055, 0.0, -0.05); g.add(pf)
...
function createStylizedDog(color) {
  ...
  return _createStylizedQuadruped(new THREE.Color(color), {

    bodyScale: [0.14, 0.09, 0.07], headSize: 0.055, headPos: [0, 0.035, -0.025],

    earType: 'floppy', tailLen: 0.07, tailCurve: 0.2, name: 'dog'
...
function createStylizedCat(color) {
  ...
  return _createStylizedQuadruped(new THREE.Color(color), {
    ...
    bodyScale: [0.12, 0.08, 0.06], headSize: 0.05, headPos: [0, 0.03, -0.02],

    earType: 'pointy', tailLen: 0.09, tailCurve: 0.7, name: 'cat'
...
function _createStylizedQuadruped(c, opts, innerEarColor) {

  const g = new THREE.Group()

  const mat = new THREE.MeshStandardMaterial({ color: c, roughness: 0.3, metalness: 0.25, emissive: c, emissiveIntensity: 0.04 })

  const innerMat = new THREE.MeshStandardMaterial({ color: new THREE.Color(innerEarColor), roughness: 0.3 })
  ...
  const [bsx, bsy, bsz] = opts.bodyScale

  // Body

  const body = new THREE.Mesh(new THREE.SphereGeometry(Math.max(bsx, bsy, bsz) * 0.7, 10, 8), mat)

  body.scale.set(bsx / 0.12, bsy / 0.08, bsz / 0.06); g.add(body)



  // Head - overlapping sphere

  const head = new THREE.Mesh(new THREE.SphereGeometry(opts.headSize, 8, 8), mat)

  head.position.set(opts.headPos[0], opts.headPos[1], opts.headPos[2]); g.add(head)



  // Snout

  const snout = new THREE.Mesh(new THREE.ConeGeometry(0.02, 0.04, 6), mat)

  snout.rotation.x = -0.15; snout.position.set(opts.headPos[0], opts.headPos[1] - 0.005, opts.headPos[2] - 0.04); g.add(snout)

  const nose = new THREE.Mesh(new THREE.SphereGeometry(0.01, 6, 6), noseMat)

  nose.position.set(opts.headPos[0], opts.headPos[1] - 0.005, opts.headPos[2] - 0.055); g.add(nose)
  ...
  for (let s = -1; s <= 1; s += 2) {
    ...
    if (opts.earType === 'pointy') {

      const ear = new THREE.Mesh(new THREE.ConeGeometry(0.022, 0.06, 4), mat)

      ear.position.set(s * 0.035, opts.headPos[1] + 0.04, opts.headPos[2] - 0.01); g.add(ear)

    } else {

      const ear = new THREE.Mesh(new THREE.SphereGeometry(0.022, 6, 6), mat)

      ear.scale.set(0.8, 0.45, 0.5)

      ear.position.set(s * 0.045, opts.headPos[1] + 0.015, opts.headPos[2] - 0.015); g.add(ear)

    }

    const ei = new THREE.Mesh(new THREE.ConeGeometry(0.01, 0.025, 4), innerMat)

    ei.position.set(s * 0.035, opts.headPos[1] + 0.045, opts.headPos[2] - 0.01); g.add(ei)

  }



  // Legs - 4 small capsules

  for (let side = -1; side <= 1; side += 2) {

    for (let front = -1; front <= 1; front += 2) {

      const leg = new THREE.Mesh(new THREE.CapsuleGeometry(0.01, 0.02, 6, 8), mat)

      leg.position.set(side * 0.045, -0.06, front * 0.06); g.add(leg)
  ...
  const tail = new THREE.Mesh(new THREE.CapsuleGeometry(0.012, opts.tailLen, 6, 8), mat)

  tail.rotation.z = opts.tailCurve * 0.5; tail.rotation.x = 0.8

  tail.position.set(0, 0.01, 0.1); g.add(tail)
  ...
  for (let s = -1; s <= 1; s += 2) {
    ...
    const e = new THREE.Mesh(new THREE.SphereGeometry(0.01, 8, 8), eyeMat)

    e.position.set(s * 0.02, opts.headPos[1] + 0.008, opts.headPos[2] - 0.04); g.add(e)

    const hl = new THREE.Mesh(new THREE.SphereGeometry(0.004, 6, 6), new THREE.MeshStandardMaterial({ color: 0xffffff }))

    hl.position.set(s * 0.022, opts.headPos[1] + 0.012, opts.headPos[2] - 0.042); g.add(hl)
  ...
  g.userData.animalType = opts.name
...
const DESK_POSITIONS = [
  ...
  { role: 'orchestrator', x: 0, z: 0, label: '???' },

  { role: 'calculator', x: -3, z: 2.5, label: '???' },

  { role: 'weather', x: 3, z: 2.5, label: '???' },

  { role: 'searcher', x: -3, z: -2.5, label: '???' },

  { role: 'summarizer', x: 3, z: -2.5, label: '???' },
...
function updateAgentStatus(role, status) {
  ...
  // Update animal colors (all children marked isAnimal or parts of animal group)

  const animalGroup = d.group.children.find(function(c) { return c.userData && c.userData.animalType })

  if (animalGroup) {

    animalGroup.children.forEach(function(part) {
...
function initScene() {
  ...
  DESK_POSITIONS.forEach(function(dp) {
    ...
    const animalFn = { owl: createStylizedOwl, fox: createStylizedFox, dolphin: createStylizedDolphin, dog: createStylizedDog, cat: createStylizedCat }[ANIMALS[dp.role]]

    const animalGroup = animalFn(AGENT_COLORS[dp.role])

    animalGroup.scale.setScalar(0.85)

    animalGroup.position.y = 0.02

    group.add(animalGroup)



    // Floating glow ring below animal

    const glowMat = new THREE.MeshBasicMaterial({

      color: agColor, transparent: true, opacity: 0.1,

      blending: THREE.AdditiveBlending, side: THREE.DoubleSide,

    })

    const glowRing = new THREE.Mesh(new THREE.RingGeometry(0.06, 0.22, 20), glowMat)
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const pt = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: agColor, transparent: true, opacity: 0.15 + Math.random() * 0.15, blending: THREE.AdditiveBlending })

      )

      const angle = (i / 4) * Math.PI * 2

      pt.position.set(Math.cos(angle) * 0.18, Math.sin(angle * 2) * 0.08, Math.sin(angle) * 0.18)

      pt.userData.orbitAngle = angle

      pt.userData.orbitSpeed = 0.3 + Math.random() * 0.2

      pt.userData.orbRadius = 0.15 + Math.random() * 0.08
    ...
    // Status ring (on the floor below)
    ...
    statusRing.userData.isStatusRing = true
    ...
    const animalEmoji = { orchestrator: '??', calculator: '??', weather: '??', searcher: '??', summarizer: '??' }[dp.role] || ''

    labelDiv.innerHTML = '<div style="text-align:center;font-size:11px;font-weight:600;color:#e8e8f0;text-shadow:0 0 12px rgba(0,0,0,0.9);background:linear-gradient(135deg,rgba(12,12,28,0.85),rgba(24,18,40,0.85));padding:5px 12px;border-radius:10px;border:1px solid ' + AGENT_COLORS[dp.role] + '55;box-shadow:0 0 16px ' + AGENT_COLORS[dp.role] + '22;">' + animalEmoji + ' ' + dp.label + '<br><span class="label-status" style="font-size:9px;color:#4ade80;">● 在线</span></div>'
...
function animate() {
  ...
  Object.keys(agentData).forEach(function(role) {
    ...
    // Animal animation - gentle idle motion

    // Find the animal group (first child with animalType)

    const ag = grp.children.find(c => c.userData && c.userData.animalType)

    if (ag) {

      // Gentle body bob

      ag.position.y = 0.02 + Math.sin(time * 0.8 + floatOffset) * 0.02

      // Type-specific animations

      const at = ag.userData.animalType

      if (at === 'owl') {

        ag.rotation.y += delta * 0.15  // slow turn

      } else if (at === 'fox' || at === 'dog' || at === 'cat') {

        // Tail wag

        const tail = ag.userData.tail

        if (tail) {

          tail.rotation.z += Math.sin(time * 2.5 + floatOffset) * delta * 0.8
        ...
        ag.rotation.z = Math.sin(time * 0.6 + floatOffset) * 0.04

        ag.rotation.x = Math.sin(time * 0.4 + floatOffset) * 0.03
    ...
    const glows = grp.children.filter(function(c) { return c.type === 'Mesh' && c.isRingGeometry })

    const glowRing = grp.children.find(function(c) { return c.type === 'Mesh' && c.geometry && c.geometry.type === 'RingGeometry' && c.material.opacity < 0.3 })

    // Actually find by position: the ring at y=-0.14

    grp.children.forEach(function(child) {

      if (child.position && Math.abs(child.position.y + 0.14) < 0.01 && child.material && child.material.blending === THREE.AdditiveBlending) {

        child.material.opacity = 0.06 + Math.sin(time * 1.2 + floatOffset) * 0.04

      }

      // Status ring

      if (child.userData && child.userData.isStatusRing) {
    ...
    if (pg) {
      ...
      pg.children.forEach(function(pt) {
        ...
        const oa = pt.userData.orbitAngle || 0

        const os = pt.userData.orbitSpeed || 0.3

        const rad = pt.userData.orbRadius || 0.18

        const a = oa + time * os

        pt.position.x = Math.cos(a) * rad

        pt.position.z = Math.sin(a) * rad

        pt.position.y = Math.sin(a * 2 + floatOffset) * 0.08
    ...
    // --- Pulse ring (status ring in group) ---

    const statusRing = grp.children.find(function(c) { return c.userData && c.userData.isStatusRing })
    ...
    if (statusRing) {
      ...
      if (d.currentStatus === 'speaking' || d.currentStatus === 'tool_calling') {
        ...
        statusRing.material.opacity = 0.3 + Math.sin(time * 3) * 0.2

        statusRing.scale.setScalar(1 + Math.sin(time * 2.5) * 0.03)
    ...
    // Legacy floor ring (keep for compatibility)
